 ...
 *******************************************************************************/
package algorithm.reduction.neighbor;

import java.util.List;
import Utils.MachineLearning.math.math.Math;
import Utils.MachineLearning.math.sort.HeapSelect;
...
1 se construye dos vectores: lower que contiene los menores valores de cada columna y upper que contiene los mayores valores
2 con estos vectores puedo obtener el radio de cada columna (upperBound[i] - lowerBound[i]) / 2, la  i es el indice de la columna 
que obtuo el radio maximo de todas las columnas
3 con lower y upper tambien se puede obtener el valor promedio(node.cutoff)) entre el minimo y el maximo valor de la columna 
ganadora ((upperBound[i] + lowerBound[i]) / 2;)
4 se ordena en un vector idex los indices de acuerdo a sus valores, en donde los valores menores al valor promedio(node.cutoff) 
se ubican a la izquierda y los valores myaores a la derecha
5 se repite el proceso construyendo el arbol de forma recursiva hasta que un nodo no tenga ni lower ni upper, osea, 
...
public class KDTree <E> implements NearestNeighborSearch<double[], E>, KNNSearch<double[], E>, RNNSearch<double[], E> {
    ...
    public KDTree(double[][] key, E[] data) {
        ...
        int n = key.length;
        index = new int[n];
        // llena el vector de indices con numeros consecutivos 1, 2, 3 ... hasta el numero de registros
        for (int i = 0; i < n; i++) {
            index[i] = i;
        }
    ...
    private Node buildNode(int begin, int end) {
        ...
        //lowerBound vector que tiene los minimos de cada columna
        double[] lowerBound = new double[d];
        //upperBound vector que tiene los maximos de cada columna
        double[] upperBound = new double[d];

        //***Realiza el proceso de asignación del vector de menores y mayores valores
        for (int i = 0; i < d; i++) {
        ...
        ///***fin de proceso de asignación

        // Calculate bounding box stats: calcula el maximo radio general
        double maxRadius = -1;
        for (int i = 0; i < d; i++) {
            double radius = (upperBound[i] - lowerBound[i]) / 2;
            if (radius > maxRadius) {
                maxRadius = radius;
                // node.split es el indice de la columna que tuvo el maximo radio
                node.split = i;
                // node.cutoff es el promedio entre el menor y mayor valor de la columna que tiene el maximo radio
                node.cutoff = (upperBound[i] + lowerBound[i]) / 2;
            }
        }

        // If the max spread is 0, make this a leaf node : para saber si es una hoja
        if (maxRadius == 0) {
        ...
        //index es el indice de la fila ganadora(maximo radio)
        //size es la posicion donde virtualmente se encontraria el valor medio de la columna(node.cutoff)
        //**** proceso que permite ordenar el vector de indices en torno al 
        //valor medio obtenido(node.cutoff), colocando los menores valores a la 
        //izquierda y los mayores valores a la derecha del dicho punto medio
        
        // Proceso que ordena los indices segun un punto de particion promedio respecto a la columna ganadora(mayor radio)
        ...
        while (i1 <= i2) {
            ...
            //si los dos valores estan en desorden respecto al valor medio de la columna que obtuvo el mayor radio, intercambia sus indices
            if (!i1Good && !i2Good) {
                int temp = index[i1];
                index[i1] = index[i2];
                index[i2] = temp;
            ...
            // si el valor que viene por la izquierda SI corresponde, entonces el indice avanza de iz a der
            if (i1Good) {
                i1++;
                size++;
            }
            // si el valor que viene por la derecha SI corresponde, entonces el indice avanza de der a iz
            if (i2Good) {
                i2--;
            }
        }
        ...
        // sigue construyendo el arbol por la izquierda
        node.lower = buildNode(begin, begin + size); // le dice de donde hasta donde lo construye por la parte iz
        // sigue construyendo el arbol por la derecha
    ...
    @Override
    //en q llegan una fila de los datos
    public Neighbor<double[], E>[] knn(double[] q, int k) {
        ...
        //Neighbor(llave, valor, indice, distancia)
        //se envian valores iniciales, en cuanto a la distancia se toma la mayor distancia posible Double.MAX_VALUE = 1.797693....
        Neighbor<double[], E> neighbor = new Neighbor<double[], E>(null, null, 0, Double.MAX_VALUE);
        @SuppressWarnings("unchecked")
        // crea una matriz de vecinos
        Neighbor<double[], E>[] neighbors = (Neighbor<double[], E>[]) java.lang.reflect.Array.newInstance(neighbor.getClass(), k); // le pasa como parametros la clase de los objetos y el tamaño
        // apila la matriz para almacenar los valores más pequeños para realizar un seguimiento. clase propia
        ...
        //busca el dato en el arbo kd y el monticulo
        search(q, root, heap);
        heap.sort();
